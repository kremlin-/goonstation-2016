	...
	proc
		...
		sample_air(var/datum/gas_mixture/G, var/no_archived)
			...
			if(no_archived)
				...
				if(G.oxygen_archived) ret["o2"] = G.oxygen_archived
				if(G.toxins_archived) ret["toxins"] = G.toxins_archived
				if(G.carbon_dioxide_archived) ret["co2"] = G.carbon_dioxide_archived
				if(G.nitrogen_archived) ret["n2"] = G.nitrogen_archived
		...
		kremdebug()
			...
			for(var/mob/M in nearby)
				if(M.client)
					ehjax.send(M.client, "reactorstats", list("firebug" = 1))
			#endif
